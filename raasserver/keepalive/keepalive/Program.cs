...
namespace Elbitin.Applications.RAAS.RAASServer.KeepAlive
{
    static class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            bool result;
            mutex = new System.Threading.Mutex(true, "keepalive", out result);
            if (!result)
                return;
            Log.Logger = RAASServerLogHelper.CreateDefaultRAASServerLogger("keepalive");
            AppDomain.CurrentDomain.FirstChanceException += OnFirstChanceException;
            Application.ApplicationExit += new EventHandler(OnApplicationExit);
            Application.EnableVisualStyles();
            Application.SetCompatibleTextRenderingDefault(false);
            Application.Run(new KeepAliveForm());
        }
