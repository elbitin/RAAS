...
namespace Elbitin.Applications.RAAS.RAASServer.Autostart
{
    static class Program
    {
        ...
        {
            bool result;
            mutex = new System.Threading.Mutex(true, "raasauto", out result);
            if (!result)
                return;
            Log.Logger = RAASServerLogHelper.CreateDefaultRAASServerLogger("raasauto");
            AppDomain.CurrentDomain.FirstChanceException += OnFirstChanceException;
            Application.ApplicationExit += new EventHandler(OnApplicationExit);
            Application.EnableVisualStyles();
            Application.SetCompatibleTextRenderingDefault(false);
            Application.Run(new AutostartForm());
        }
