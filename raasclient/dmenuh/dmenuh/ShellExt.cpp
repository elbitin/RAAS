...
BOOL CShellExt::_ServerExists(wchar_t* szServerName, wchar_t* serversFile)
{
	BOOL fExists = FALSE;
	HRESULT hr = S_OK;
	...
	CHK_HR(VariantFromString(_bstr_t(serversFile), varFileName));
...
void CShellExt::_CreateRemoteShortcut()
{
	...
	if (SUCCEEDED(hr))
	{
		if (szInstallDir == NULL)
			return;
		if (this->m_szDir[lstrlenW(this->m_szDir) - 1] == '\\')
			this->m_szDir[lstrlenW(this->m_szDir) - 1] = NULL;
		if (SUCCEEDED(StringCchPrintf(szPath, ARRAYSIZE(szPath),
			L"\"%s\" \"%s\"", this->m_szFile, this->m_szDir)) &&
			SUCCEEDED(StringCchPrintf(szRemoteScPath, ARRAYSIZE(szRemoteScPath),
				L"\"%sremotesc.exe\"", szInstallDir)))
		{
...
STDMETHODIMP CShellExt::Initialize(
	LPCITEMIDLIST pidlFolder, LPDATAOBJECT pDatoObj, HKEY hkeyProgID)
{
	SHGetPathFromIDList(pidlFolder, m_szDir);
	FORMATETC fmt = { CF_HDROP, NULL, DVASPECT_CONTENT, -1, TYMED_HGLOBAL };
	STGMEDIUM stg = { TYMED_HGLOBAL };
	HDROP     hDrop;
	if (FAILED(pDatoObj->GetData(&fmt, &stg)))
	{
		return E_INVALIDARG;
	}
	...
	UINT idCmdLast, UINT uFlags)
{
	...
	if (PathIsDirectory(this->m_szFile))
		return MAKE_HRESULT(SEVERITY_SUCCESS, FACILITY_NULL, 0);
	WCHAR szRemoteShortcutMenuItem[80];
	LoadString(g_hInst, IDS_REMOTESHORTCUTHERE, szRemoteShortcutMenuItem, ARRAYSIZE(szRemoteShortcutMenuItem));
	InsertMenu(hmenu, indexMenu++, MF_BYPOSITION, idCmdFirst + IDM_REMOTE_SHORTCUT, szRemoteShortcutMenuItem);
	return MAKE_HRESULT(SEVERITY_SUCCESS, FACILITY_NULL, IDM_REMOTE_SHORTCUT + 1);
}
	...
	UINT_PTR idCmd, UINT uFlags, UINT* pwRes, LPSTR pszName, UINT cchMax)
{
	...
	{
		wchar_t* szText = _T("RAAS Client context menu commands");
